...
class Homepage extends CI_Controller {
	...
	public function submitData(){
		...
		if(!$this->loginSession){
			$this->form_validation->set_rules('userFullName','Username', 'trim|required|alpha_numeric|min_length[2]|max_length[20]|is_unique[logintbl.fullName]', array('is_unique' => 'O %s já está em uso'));
			$this->form_validation->set_rules('userEmail', 'Email', 'trim|required|valid_email|is_unique[logintbl.email]', array('is_unique' => 'Você já está cadastrado com este %s. Por favor, faça o login primeiro para mais reservas.'));
