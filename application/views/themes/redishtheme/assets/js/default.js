					...
					success: function(data) {
						if(data.success == false){
							...
							if(data.agents.length > 1){
								agents += '<label class="agentItem" id="anyAgent"><input type="radio" value="0" name="agent" user_data="'+data.agents.length+'" label="Any Agent" class="viewDetail" checked="checked"><div class="agentItemInner"><div class="avatar"><div class="avatarBg" style="background-image: url('+upimg+'agents/default-avatar.jpg)"></div></div><div class="avatarName">Aleatório</div></div></label>';
							}
							$.each(data.agents, function(i, agent) {
								
								agents += '<label class="agentItem agentsAnim"><input id="agentRad" type="radio" value="'+agent.id+'" name="agent" label="'+agent.agentName+'" class="viewDetail"><div class="agentItemInner"><div class="avatar"><div class="avatarBg" style="background-image: url('+upimg+'agents/'+agent.agentImage+')"></div></div><div class="avatarName">'+agent.agentName+'</div><a class="agentModalOpen viewDetail"><span>Ver detalhes</span></a></div><div class="agentModal"><div class="agentModalInner"><span class="closeModalBtn">Fechar Detalhes <i class="icon-close"></i></span><div class="agentModalHeader"><img src="'+upimg+'agents/'+agent.agentImage+'" class="img-thumbnail rounded-circle"><h3>'+agent.agentName+'</h3></div><div class="userColumns"><div class="row"><div class="col-md-6"><h4>'+agent.experience+'</h4><span>Experience Years</span></div><div class="col-md-6"><h4>'+agent.totalBookings+'</h4><span>Served Customers</span></div></div></div><p class="agentDetails">'+agent.agentDetail+'</p></div></div></label>';
							});
							$('.selectAgentMain').append(agents);
							if(incd == 0) {
								$userSelectTiming.prop('disabled', true);
								$('#notimeError').removeClass('d-none');
								$userSelectTiming.empty().append('<option value="">Selecionar horários</option>');
								...
								$datepicker.on('dp.show dp.change', function() {
									...
									if(today != dateToday) {
										...
										$userSelectTiming.empty().append('<option value="">Selecionar horário</option>').append(timing);
										$('#notimeError').addClass('d-none');
									} else {
										$userSelectTiming.prop('disabled', true);
										$userSelectTiming.empty().append('<option value="">Selecionar horário</option>');
										$('#notimeError').removeClass('d-none');
									}
								});
								...
								$datepicker.on('dp.show dp.change', function() {
									...
									if(today != dateToday) {
										...
										$userSelectTiming.prop('disabled', false);
										$userSelectTiming.empty().append('<option value="">Selecionar horário</option>').append(todayOpt);
									}
								})

								$userSelectTiming.prop("disabled", false);
								$userSelectTiming.empty().append('<option value="">Selecionar horário</option>').append(opt);
							}
